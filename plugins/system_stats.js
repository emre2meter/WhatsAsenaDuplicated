...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({pattern: 'alive', fromMe: true, dontAddCommandList: true, desc: Lang.ALIVE_DESC}, (async (message, match) => {
        ...
        if (Config.ALIVEMSG == 'default') {
            await message.client.sendMessage(message.jid, fs.readFileSync("/root/WhatsAsenaDuplicated2/media/gif/alive.mp4"),MessageType.video, { mimetype: Mimetype.mp4, caption: '```CALYPSO Founder 〆𝐘𝐈𝐊𝐈𝐊〆 İçin Çalışıyor```'});
        }
        else {
            await message.client.sendMessage(message.jid, fs.readFileSync("/root/WhatsAsenaDuplicated2/media/gif/alive.mp4"),MessageType.video, { mimetype: Mimetype.mp4, caption: '```CALYPSO Founder 〆𝐘𝐈𝐊𝐈𝐊〆 İçin Çalışıyor```'});
        }
